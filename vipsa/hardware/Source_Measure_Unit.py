...
class KeithleySMU():
    ...
    def run_read_probe(self, read_vlist, compliance=1e-3, delay=1e-3, nplc=0.01, label="probe",
                       progress_callback=None, stream_chunk=25, current_autorange=False):
        """
        ...
        if current_autorange:
            ...
            self.smu.write(":SENS:CURR:RANG:AUTO OFF")
            self.smu.current_range = max(1e-9, compliance * 10)
    ...
    def list_IV_sweep_split_4(
        ...
        current_autorange=False,
            ...
            for seg_no, tag, vlist_seg, idx0, idx1 in splits:
                ...
                range_desc = "AUTO" if current_autorange else f"{self.smu.current_range} A"
                print(f"\nSeg {seg_no}: {tag} | points={len(vlist_seg)} | comp={comp} A | I_range={range_desc}")
                    ...
                    chunk = self.run_linear_segment(
                        {
                            **s,
                            "current_autorange": current_autorange,
                            "manual_range": meas_range_map[tag],
                        },
                        compliance=comp,
                        delay_input=delay,
                        progress_callback=progress_callback,
    ...
    def run_linear_segment(self, seg, compliance, delay_input = None, progress_callback=None, stream_chunk=25):
        ...
        current_autorange = seg.get("current_autorange", False) if isinstance(seg, dict) else False
        manual_range = seg.get("manual_range") if isinstance(seg, dict) else None
        ...
        if current_autorange:
            ...
            self.smu.write(":SENS:CURR:RANG:AUTO OFF")
            self.smu.current_range = manual_range if manual_range is not None else compliance * 10
