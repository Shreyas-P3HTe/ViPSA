...
from vipsa.hardware.Openflexture import Light  # Import the Light class
import threading
from vipsa.analysis.Vision import overlay
import csv
import math
from vipsa.hardware.Source_Measure_Unit import KeysightSMU, pyvisa
import pandas as pd
...
def make_grid(x_distance, y_distance, rows, cols, save_path, tilt_angle=0):
    ...
    Args:
        ...
        tilt_angle (float): Tilt angle in degrees (default: 0Â°).
    """
    
    try :
        # Get the starting position from Zaber stages
        if zaber_x_axis == None or zaber_y_axis == None :
            connect_to_zaber('COM6')
        ...
        print(f"Grid with tilt angle {tilt_angle}Â° saved to {save_path}")
        
    except Exception as e :
        
        sg.popup("Problem occured :", e)
...
def single_measurement(sample_no, device_no, pos_compl, neg_compl, sweep_delay, approach, sweep_path, save_path) :
    
    SMU = KeysightSMU(0)
        
    Vipsa_Methods().run_single_DCIV(sample_no, device_no, pos_compl, neg_compl, sweep_delay, SMU=SMU,
                                    stage=stage_obj, Zaber_x=zaber_x_axis, Zaber_y=zaber_y_axis, top_light=light_controller,
                                    approach=approach, sweep_path=sweep_path, save_directory=save_path,
                                    align=False, )
...
def main():
    ...
    grid_tab = [
        ...
        sg.Column([
            ...
            [sg.Text("Grid save Path:"), sg.InputText(key='-GRID_PATH-', size=(50, 1)), sg.FolderBrowse()],
    ...
    measure_tab = [
        ...
        [sg.Button("Connect SMU", key='-CONNECT_SMU-')],
        ...
        [sg.Text("Save path : "), sg.InputText(key='-SAVE_PATH-', size=(50, 1)), sg.FolderBrowse()],
        
        
        [sg.HorizontalSeparator()],
        
        [sg.Text("Positive compliance :"),sg.InputText(default_text='0.001', key='-POSITIVE_COMPLIANCE-', size=(10, 1)),
         sg.Text("Negative compliance :"),sg.InputText(default_text='0.01', key='-NEGATIVE_COMPLIANCE-', size=(10, 1))],
        [sg.Text("Sweep delay :"), sg.InputText(default_text='0.0001', key='-SWEEP_DELAY-', size=(10,1))],
        [sg.Text("Sweep path : "), sg.InputText(key='-SWEEP_PATH-', size=(50, 1)), sg.FileBrowse()],
        [sg.Button("Run Single DCIV Measurement", key='-RUN_SINGLE_MEASUREMENT-')],
        
        [sg.HorizontalSeparator()],

        [sg.Text("Positive compliance :"),sg.InputText(default_text='0.001', key='-POSITIVE_COMPLIANCE-', size=(10, 1)),
         sg.Text("Negative compliance :"),sg.InputText(default_text='0.01', key='-NEGATIVE_COMPLIANCE-', size=(10, 1))],
        [sg.Text("Pulse delay :"), sg.InputText(default_text='0.0001', key='-SWEEP_DELAY-', size=(10,1))],
        [sg.Text("Pulse path : "), sg.InputText(key='-SWEEP_PATH-', size=(50, 1)), sg.FileBrowse()],
        [sg.Button("Run Single Pulsed Measurement", key ='-RUN_SINGLE_PULSE-')]
    ...
    while True:
        ...
        if event == '-CONNECT_LIGHTS-':
            ...
            light_controller.control_lights('rainbow')
        ...
        if event == '-CREATE_GRID-':
            ...
            grid_path = values['-GRID_PATH-']
            make_grid(x_dist, y_dist, rows, cols, grid_path, tilt)
        ...
        if event == '-RUN_SINGLE_MEASUREMENT-':
            ...
            pos_compl = float(values['-POSITIVE_COMPLIANCE-'])
            neg_compl = float(values['-NEGATIVE_COMPLIANCE-'])
            sweep_file = values['-SWEEP_PATH-']
            save_folder = values['-SAVE_PATH-']
            sweep_delay = float(values['-SWEEP_DELAY-'])
            
            if values['-CONTACTED-'] :                
                single_measurement(sample_no, device_no, pos_compl, neg_compl, sweep_delay, approach = False, save_path=save_folder, sweep_path=sweep_file)
                
            elif values['-NOT_CONTACTED-']:
                single_measurement(sample_no, device_no, pos_compl, neg_compl, sweep_delay, approach= True, save_path=save_folder, sweep_path=sweep_file)
