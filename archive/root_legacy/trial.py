...
def main():
    ...
    for seg_no, tag, vlist, i0, i1 in splits:
        print(
            ...
            f"Vstart={vlist[0]: .3f} V → Vend={vlist[-1]: .3f} V"
        )
    ...
    if max(counts.values()) - min(counts.values()) <= 1:
        print("  ✔ pf/pb/nf/nb counts roughly balanced")
    else:
        print("  ✘ imbalance detected (likely split bug)")

    if any(len(vlist) > 1500 for _, _, vlist, _, _ in splits):
        print("  ✘ found abnormally large segment (likely missed turning point)")
    else:
        print("  ✔ no runaway segments")
