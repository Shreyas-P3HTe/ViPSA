...
class ArtifactSaveTests(unittest.TestCase):
    ...
    def test_save_file_accepts_numpy_object_array_of_measurement_dicts(self):
        handler = Data_Handler()
        tmpdir = os.path.join(PACKAGE_ROOT, "tests_artifacts_tmp")
        os.makedirs(tmpdir, exist_ok=True)
        try:
            ...
            self.assertEqual(list(df["Voltage (V)"]), [0.0, 0.5])
            self.assertAlmostEqual(df["Current (A)"].iloc[1], 1e-6)
            self.assertNotIn("Unnamed: 0", df.columns)
        finally:
            shutil.rmtree(tmpdir, ignore_errors=True)

    def test_build_measurement_figure_recovers_marshaled_dict_cells(self):
        handler = Data_Handler()
        tmpdir = os.path.join(PACKAGE_ROOT, "tests_artifacts_tmp")
        os.makedirs(tmpdir, exist_ok=True)
        try:
            bad_rows = pd.DataFrame(
                [
                    {
                        "Time(T)": "{'Time(T)': 0.0, 'Voltage (V)': 0.0, 'Current (A)': 1e-09, 'V_cmd (V)': 0.0, 'V_meas (V)': nan, 'V_error (V)': nan, 'Cycle Number': 1}",
                        "Voltage (V)": "{'Time(T)': 0.1, 'Voltage (V)': 0.5, 'Current (A)': 1e-06, 'V_cmd (V)': 0.5, 'V_meas (V)': nan, 'V_error (V)': nan, 'Cycle Number': 1}",
                        "Current (A)": "{'Time(T)': 0.2, 'Voltage (V)': 0.0, 'Current (A)': 2e-09, 'V_cmd (V)': 0.0, 'V_meas (V)': nan, 'V_error (V)': nan, 'Cycle Number': 1}",
                        "V_cmd (V)": "{'Time(T)': 0.3, 'Voltage (V)': -0.5, 'Current (A)': -1e-06, 'V_cmd (V)': -0.5, 'V_meas (V)': nan, 'V_error (V)': nan, 'Cycle Number': 1}",
                        "V_meas (V)": "{'Time(T)': 0.4, 'Voltage (V)': 0.0, 'Current (A)': 3e-09, 'V_cmd (V)': 0.0, 'V_meas (V)': nan, 'V_error (V)': nan, 'Cycle Number': 1}",
                        "V_error (V)": "{'Time(T)': 0.5, 'Voltage (V)': 0.25, 'Current (A)': 5e-07, 'V_cmd (V)': 0.25, 'V_meas (V)': nan, 'V_error (V)': nan, 'Cycle Number': 1}",
                        "Cycle Number": 1,
                        "Contact Current (A)": 0.013,
                        "Z position": -4.0,
                    }
                ]
            )
            csv_path = os.path.join(tmpdir, "marshaled.csv")
            ...
            self.assertGreater(len(line.get_xdata()), 1)
            self.assertTrue(np.isfinite(np.asarray(line.get_xdata(), dtype=float)).all())
        finally:
            shutil.rmtree(tmpdir, ignore_errors=True)
